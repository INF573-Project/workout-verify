...
class ForwardStageInference(IForwardStage[DataStageOutputLoad, DataStageInference, ForwardStageJoints]):
    ...
    def process_one_image(self, img, detector, pose_estimator, visualizer=None):
        # predict bbox
    ...
    def process_hand(self, video_path):
    ...
    def compute(self) -> None:
        ...
        if not isfile(kpts_joints_path) or not isfile(kpts_hands_path):
            ...
            results_hand = self.process_hand(self.input.video_path)
            
            kpts_joints = []
            kpts_hands = []

            for result in results_hand:
                kpts_hands.append(result)
    ...
    def get_output(self) -> Tuple[ForwardStagehands, DataStageInference]:
